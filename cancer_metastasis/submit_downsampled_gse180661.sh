#!/bin/bash
# Submit the depth-equalised GSE180661 pair array, in one of three
# configurations.
#
...
#   mismatched  negative control: each primary against a *different* patient's
#               metastasis, under the same unconstrained rejection as `free`,
#               so the two retained fractions are directly comparable. Build
#               its manifest first with 28_build_mismatched_manifest.py.
#
# Usage:
...
config=${1:-}
case "$config" in
  free|bounded|mismatched) ;;
  *) echo "Usage: $0 {free|bounded|mismatched}" >&2; exit 2 ;;
esac
...
if [[ "$config" == "mismatched" ]]; then
  export CONFIDENCEOT_MANIFEST="$downsampled/pair_manifest_mismatched.csv"
else
...
if [[ "$config" == "bounded" ]]; then
  ...
  # The control must be read against `free`, so it is configured identically.
