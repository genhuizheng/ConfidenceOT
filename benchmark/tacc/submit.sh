...
if [[ "$stage" == "all" || "$stage" == "ot" ]]; then
    ...
    ot_id=$(submit "OT ($chunks chunks of $chunk units)" \
        --array=0-$(( chunks - 1 )) \
        "${depend[@]}" \
        --export=ALL,CONFIDENCEOT_BENCH_ROOT="$root",CONFIDENCEOT_REPO="$repo",CONFIDENCEOT_BENCH_CHUNK="$chunk",CONFIDENCEOT_BENCH_WORKLIST="$worklist" \
