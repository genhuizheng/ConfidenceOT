"""Turn one Splatter replicate into the benchmark's conditions.

Three technical levels crossed with three biological cases, from a single
generated replicate, written as the h5ad pairs and the manifest that
``cancer_metastasis/02_run_pair.py`` already knows how to read.

**The levels are stages, not axes.** L0 leaves the target alone. L1 thins the
target's reads, which moves nCount and takes detection with it. L2 keeps that
thinning and adds dropout, which moves both again. There is no level that
holds one readout while moving the other, because at a fixed library size
nothing can change how many genes are detected except changing what the counts
land on -- and that is a change to the cell, not to the measurement.

Thinning is binomial, the exact likelihood of sequencing the same library less
deeply: every read survives independently with probability theta. Splatter has
no per-batch library size, so this is the one step that cannot be expressed in
the simulator, and it is the only post-hoc operation on the counts.

Dropout is a per-gene Bernoulli mask weighted towards the low expressers,
using the gene means Splatter reported. Weighted too steeply it removes almost
no counts -- the low expressers carry little of the library -- and the target
arrives with its total intact, which is the fixed-total construction this
design rejects. The tilt used here costs the top of the distribution about
half of what it costs the bottom, so both readouts move.

**The cases.** ``all_shared`` keeps every group on both sides. ``population_lost``
removes the last group from the target, so the source cells of that group have
no match. ``population_emerged`` removes it from the source instead. Both
directions are built, because the gate is called on both sides and a benchmark
that only ever asks one side to reject cannot see half of what it claims to
measure.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--generated", type=Path, required=True,
                        help="Directory written by 10_generate_splatter.R")
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--n-cells", type=int, required=True)
    parser.add_argument("--replicate", type=int, required=True)
    parser.add_argument("--theta", type=float, default=0.50,
                        help="Read survival probability for the target at L1 "
                             "and L2; the realised depth ratio is measured, "
                             "not assumed")
    parser.add_argument("--dropout", type=float, default=0.55,
                        help="Strength of the extra per-gene loss at L2")
...
def read_generated(root: Path) -> tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
    """Genes x cells from R, returned cells x genes."""
...
def drop(counts: np.ndarray, gene_mean: np.ndarray, strength: float,
         rng: np.random.Generator) -> np.ndarray:
    """Per-gene capture loss, tilted towards the low expressers."""
    if strength <= 0.0:
        return counts
    weight = 0.45 + 0.55 / (1.0 + gene_mean / np.median(gene_mean))
    keep = rng.random(counts.shape) > strength * weight
    return counts * keep
...
def write_side(path: Path, counts: np.ndarray, cells: pd.DataFrame,
    ...
    data.uns["metadata"] = {"expression_kind": "raw_counts",
...
def main() -> None:
    args = parse_args()
    counts, cells, genes = read_generated(args.generated)
    rng = np.random.default_rng(args.seed + 7919 * args.replicate)
    ...
    gene_mean = genes["gene_mean"].to_numpy(dtype=np.float64)
    rows = []
    for level in LEVELS:
        target = counts[target_mask]
        if level != "L0_matched":
            target = thin(target, args.theta, rng)
        if level == "L2_depth_dropout":
            target = drop(target, gene_mean, args.dropout, rng)
        ...
        for case in CASES:
            ...
            pair_id = (f"N{args.n_cells}_rep{args.replicate}_{level}_{case}")
            ...
            # inference: the group label came out of the simulator. The side
            # that rejects is the side that still *has* the group, which is
            # the opposite of the side the removal was applied to.
            ...
            (directory / "readouts.json").write_text(json.dumps(
                {"source": source_read, "target": target_read,
                 "measured": measured, "theta_nominal": args.theta,
                 "dropout_nominal": args.dropout if level == "L2_depth_dropout" else 0.0,
                 "unmatched_group": unmatched,
                 "rank_top_n": args.rank_top_n,
                 "rank_cut_valid": bool(
                     min(source_read["min_nFeature"],
            ...
            rows.append({
                ...
                "unmatched_group": unmatched,
                "theta_nominal": args.theta,
                "dropout_nominal": (args.dropout
                                    if level == "L2_depth_dropout" else 0.0),
    ...
    print(f"wrote {len(table)} conditions to {args.out / 'manifest.csv'}")
    if invalid:
